 ...
 * @testCase
 */
class DocumentsPresenterTest extends TestCase
{
    /** @var Container  */
    private $container;
    ...
    {
        $client = new Client;
        $response = $client->get($this->server->getUrl() . 'v2/documents', [
            'cookies' => $this->cookieJar
        ]);
        ...
        try {
            ...
            Assert::type('array', $res->documents);
...
$testCase = new DocumentsPresenterTest($container);
