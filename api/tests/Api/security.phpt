 ...
 * @testCase
 */
class SecurityPresenterTest extends TestCase
{
    /** @var Container  */
    private $container;
    ...
    {
        $client = new Client;
        $response = $client->get($this->server->getUrl() . 'v2/security/logout', [
            'cookies' => $this->cookieJar
        ]);
        ...
        try {
            ...
            Assert::type('bool', $res->message);
            Assert::same(true, $res->message);
    ...
    {
        $client = new Client;
        $response = $client->get($this->server->getUrl() . 'v2/security/profile', [
            'cookies' => $this->cookieJar
        ]);
...
$testCase = new SecurityPresenterTest($container);
